<script>
    ...
    import { Button, ButtonGroup, InputAddon, Navbar, NumberInput } from 'flowbite-svelte';
...
<Navbar class="p-4 border">
    ...
    <ButtonGroup >
        <Button size="sm" class="px-2 font-bold" on:click={() => scale = scale - 0.1}>-</Button>
        <NumberInput size="sm" bind:value={scale} class="w-10"/>
        <Button size="sm" class="px-2 font-bold" on:click={() => scale = scale + 0.1}>+</Button>
    </ButtonGroup>
...
<div class="flex flex-col w-full min-h-dvh max-h-dvh overflow-auto items-center">
    <div bind:this={wrapper} class="relative flex flex-1 w-full items-start justify-{justifyContent} p-8 " class:tajweed={showTajweed} style="font-family: 'Digital Khatt2';">
        <div class="flex flex-col px-16 py-8 leading-loose bg-white text-black w-[148mm] origin-top transition-all text-2xl border" style="transform: scale({scale}); transform-origin: {transformOrigin};" dir=rtl>
