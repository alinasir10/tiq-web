<script>
    ...
    const loadFont = () => {
        const fontFace = new FontFace(`p${pageNumber}-v4`, `url(${fontUrl})`, {
            weight: 'normal',
            style: 'normal'
          });
...
<div class="flex flex-col w-full min-h-dvh max-h-dvh overflow-auto items-center">
    ...
    <div bind:this={wrapper} class="relative flex flex-1 w-full items-start justify-{justifyContent} p-8" style="font-family: 'p{pageNumber}-v4';">
        <div class="flex flex-col p-16 gap-4 bg-blue-50 w-[148mm] h-[210mm] origin-top text-black transition-all text-xl" style="transform: scale({scale}); transform-origin: {transformOrigin};" dir=rtl>
                {#each Array.from({ length: 15 }) as _, lineIndex}
                    <div class="flex justify-between items-center line-{lineIndex} border-b">
                            ...
                            {#each verse.words as word, index}
                                {#if word.line_number === lineIndex + 1}
                                    <div class="flex rounded items-center {hoveredVerse === verse.verse_key ? 'bg-neutral-700' : ''}">
                                        ...
                                        <!--    <span role=none class="btn btn-circle btn-xs m-0.5 {verse.verse_number % 5 === 0 ? 'btn-primary' : 'btn-accent'}">-->
                                        <!--        &lt;!&ndash; {word.text} &ndash;&gt;-->
                                        <!--        {word.code_v2}-->
                                        <!--    </span>-->
                                        <!--{:else}-->
...
<!--    @font-face {-->
<!--        font-family: 'p10-v4';-->
<!--        src: url("https://quran.com/fonts/quran/hafs/v4/colrv1/woff2/p10.woff2") format('woff2');-->
<!--        font-weight: normal;-->
<!--        font-style: normal;-->
<!--      }-->
<!--</style>-->